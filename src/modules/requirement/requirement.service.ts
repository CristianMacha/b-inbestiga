...
export class RequirementService {
  ...
  async findOne(requirementId: number): Promise<Requirement> {
    try {
      const requirementDb = await this.requirementRepository.findOne(
        requirementId,
      );
  ...
  async updateActive(requirementId: number): Promise<Requirement> {
    try {
      ...
      const requirementUpdate = await this.requirementRepository.save(
        requirementDb,
      );
  ...
  async update(requirement: Requirement): Promise<Requirement> {
    try {
      const requirementDb = await this.requirementRepository.preload(
        requirement,
      );
      if (!requirementDb) {
        throw new NotFoundException('Requirement not found.');

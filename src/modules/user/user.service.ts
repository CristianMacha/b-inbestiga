...
export class UserService {
  ...
  async create(userRegister: UserRegisterInterface): Promise<User> {
    try {
      const emailExist = await this.findByEmail(userRegister.email);
      if (emailExist) {
        throw new BadRequestException('Email ya registrado');
      }
      const connection = getConnection();
      const userCreated = await connection.transaction(
        'SERIALIZABLE',
        async (manager) => {
          const passwordHash = await this.bcryptServices.encryptPassword(
